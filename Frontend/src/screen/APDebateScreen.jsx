...
import {
  ...
  X,
  BookOpen,
  Settings, // For Setup Screen icon
} from 'lucide-react';
...
const APDebateScreen = () => {
  ...
  const debateSetup = location.state;
  ...
  useEffect(() => {
    if (debateSetup) {
      console.log("Debate Setup Received in AP Debate Screen:", debateSetup);
      setCurrentMotion(debateSetup.motion);
      setUserRole(debateSetup.userRole);

      const format = debateSetup.formatDetails;
      // Use the correct AP speaker order and teams
      ...
      console.log("AP Speaker Order:", apSpeakerOrder);
      ...
      addTranscriptEntry('Moderator', `The motion for today's debate is: "${safeDisplay(debateSetup.motion)}".`, 'info');

    } else {
      console.warn("No debate setup found for AP Debate Screen. Redirecting to setup.");
      toast.error("No debate setup found. Redirecting to setup screen.", {
        duration: 3000,
        position: 'top-center',
      });
  ...
  }, [debateSetup, navigate, addTranscriptEntry]);
      ...
      if (isPrepTime) {
        ...
        if (speakerOrder.length > 0) {
          ...
          if (currentSpeaker && currentSpeaker.role !== userRoleBase) {
            addTranscriptEntry(safeDisplay(currentSpeaker.role), `(Simulated AI Speech) Good evening, everyone. I stand here as the ${safeDisplay(currentSpeaker.role)} to begin our arguments on this motion.`, 'speech');
          }
        }
        setUserPoiAvailability(false);
      } else {
  ...
  const handleNextSpeaker = () => {
    ...
    if (isPrepTime) {
      ...
      if (nextSpeaker && nextSpeaker.role !== userRoleBase) {
        addTranscriptEntry(safeDisplay(nextSpeaker.role), `(Simulated AI Speech) Good evening, everyone. I stand here as the ${safeDisplay(nextSpeaker.role)} to begin our arguments on this motion.`, 'speech');
      }
  ...
  if (!debateSetup) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4">
        <h1 className="text-4xl text-amber-400">Loading Asian Parliamentary Debate...</h1>
      ...
      <div className="lg:w-2/5 p-6 sm:p-8 bg-gray-800 border-r border-gray-700 shadow-xl flex flex-col justify-between">
        <div>
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-6 text-center lg:text-left">
            <span className="text-emerald-400">AP</span> Debate Live
          </h1>
          ...
          {/* Debate Transcript Section - NOW ON LEFT PANE */}
        ...
        <div className="mb-8 p-6 bg-gray-800 rounded-lg border border-gray-600 shadow-lg">
          ...
          <ol className="space-y-2 text-lg">
            {speakerOrder.map((speaker, index) => {
              const displayString = `${speaker.role}${speaker.team ? ' (' + speaker.team + ')' : ''}`;
              return (
                <li
                  key={index}
                  className={`
