...
// --- MODIFICATION: Default setup data for demo purposes ---
const demoDebateSetup = {
    motion: "This House Believes That Artificial Intelligence Poses a Threat to Humanity",
    userRole: "Prime Minister (You)",
    formatDetails: {
        name: 'Asian Parliamentary (AP)',
        teams: '2 teams of 3 speakers each, plus a reply speech from each side',
        structure: 'Government and Opposition benches',
        prepTime: '15 minute prep time',
        speechTime: '7-minute speeches',
        rules: [
            'Two teams: Government and Opposition.',
            '7-minute speeches for constructive and whip roles.',
            '4-minute reply speeches.',
            'Points of Information (POIs) are allowed during the middle 5 minutes of constructive speeches.',
            'Reply speeches are a summary and cannot introduce new arguments.'
        ],
        speakerRolesDetailed: [
             { role: "Prime Minister", description: "Defines the motion, outlines the Government's case." },
             { role: "Leader of Opposition", description: "Rebuts the PM, presents Opposition's case." },
             { role: "Deputy Prime Minister", description: "Rebuts DLO and rebuilds Government case." },
             { role: "Deputy Leader of Opposition", description: "Rebuts DPM and rebuilds Opposition case." },
             { role: "Government Whip", description: "Summarizes the debate from a Government perspective." },
             { role: "Opposition Whip", description: "Summarizes the debate from an Opposition perspective." },
             { role: "Government Reply", description: "Provides a concluding, biased summary for the Government." },
             { role: "Opposition Reply", description: "Provides a concluding, biased summary for the Opposition." },
        ]
    },
};
...
// --- APDebateScreen Component ---
const APDebateScreen = () => {
    const location = useLocation();
    const navigate = useNavigate();
    // --- MODIFICATION: Use demo data as a fallback if no state is passed ---
    ...
    // POI State from original AP Screen
    ...
    useEffect(() => {
        // --- MODIFICATION: This effect now always runs with valid setup data ---
        const formatDetails = debateSetup.formatDetails;
        const apSpeakerOrder = [
            ...
            { role: 'Government Reply', team: 'Government' },
            { role: 'Opposition Reply', team: 'Opposition' },
        ];
        setSpeakerOrder(apSpeakerOrder);
        ...
        addTranscriptEntry('Moderator', `The AP debate motion is: "${safeDisplay(debateSetup.motion)}".`, 'info');
    ...
    const handleNextSpeaker = useCallback((isFirstSpeaker = false) => {
        ...
        if (nextIndex < speakerOrder.length) {
            ...
            if (debateSetup && nextSpeaker?.role !== debateSetup.userRole.split(' (')[0]) {
    ...
    }, [isPrepTime, currentSpeakerIndex, speakerOrder, speechDuration, debateSetup, addTranscriptEntry, speak, handleDebateConcluded, isListening, stopListening]);
    ...
    const isInProtectedTime = useCallback(() => {
        if (isPrepTime || !isActive || !speakerOrder[currentSpeakerIndex]) return true;
        if (speakerOrder[currentSpeakerIndex]?.role.includes('Reply')) return true;

        const elapsedSpeechTime = speechDuration - timer;
        return elapsedSpeechTime < protectedTimeDuration || timer < protectedTimeDuration;
    }, [isPrepTime, isActive, timer, speechDuration, protectedTimeDuration, currentSpeakerIndex, speakerOrder]);

    // Bell and POI Offer Logic
    useEffect(() => {
        const currentSpeaker = speakerOrder[currentSpeakerIndex];
        if (isActive && !isPrepTime && currentSpeaker) {
            if (timer === speechDuration - protectedTimeDuration && !currentSpeaker.role.includes('Reply')) {
                playBell("C5");
                toast.success("Protected time is over. POIs are now open.", { icon: '👋' });
            }
            if (timer === protectedTimeDuration && !currentSpeaker.role.includes('Reply')) {
                playBell("C5");
    ...
    }, [timer, isActive, isPrepTime, speechDuration, protectedTimeDuration, speakerOrder, currentSpeakerIndex, debateSetup, aiPoiOfferActive, isInProtectedTime, addTranscriptEntry]);
    ...
    const handleAiPoiResponse = (accepted) => {
        ...
        if (accepted) {
            ...
            addTranscriptEntry(offeringRole, "(Simulated POI: On that point, how do you account for...?)", 'poi-delivery');
        } else {
    ...
    const handleUserOfferPoi = () => {
        ...
        setTimeout(() => {
            ...
            if (aiAccepts) {
                addTranscriptEntry(currentSpeaker.role, 'Accepted.', 'poi-response');
            } else {
                addTranscriptEntry(currentSpeaker.role, 'Declined.', 'poi-response');
            }
        }, 1500);
    };
        ...
        <div className="min-h-screen bg-gray-900 text-gray-100 flex flex-col font-sans overflow-hidden">
            ...
            <main className="flex-grow grid grid-cols-1 lg:grid-cols-4 gap-6 p-6">
                <div className="lg:col-span-1 flex flex-col gap-6">
                    <TeamPanel teamName="Government" speakers={speakerOrder.filter(s => s.team === 'Government')} currentSpeaker={currentSpeaker} userRole={userRoleBase} teamColor="gov" speakingAi={speakingAi} />
                </div>
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full border border-gray-700 animate-fade-in-up flex flex-col" style={{maxHeight: '90vh'}}>
                        ...
                        <div className="p-6 overflow-y-auto">
                            <h3 className="font-bold text-xl text-emerald-300 mb-3">AP Rules</h3>
