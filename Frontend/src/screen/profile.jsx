...
const ProfilePage = () => {
  ...
  // Fetch user and gamification data on mount
  useEffect(() => {
  ...
  const currentLevelObj = levels.find(lvl => lvl.level === currentLevel) || levels[0];
  const nextLevelObj = levels.find(lvl => lvl.level === currentLevel + 1);
  const xpForNext = nextLevelObj ? nextLevelObj.xpRequired : currentLevelObj.xpRequired;
  const xpMin = currentLevelObj.xpRequired;
  const xpMax = nextLevelObj ? nextLevelObj.xpRequired : xpMin + 1000;
  const progress = ((currentXP - xpMin) / (xpMax - xpMin)) * 100;
